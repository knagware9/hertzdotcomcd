...
type Driver struct{
	...
	DL string `json:"dl"`					//the fieldtags are needed to keep case from bouncing around
...
func (t *SimpleChaincode) signup_driver(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//build the marble json string manually
	str := `{"name": "` + name + `", "dl": "` + dl + `","email": "` + email + `",  "mobile": "` + mobile + `", "password": "` + password + `","address": "` + address + `","status": "` + status + `"}`
